...
- `ts` باید در بازهٔ **±۵ دقیقه** از ساعت سکو باشد (`400 stale`). ساعت سرور را با NTP هم‌زمان نگه دارید.
...
- `scope`: رشته‌ای که قراردادهای بعدی زیر این وکالت باید با آن بخوانند: `template_ref` یا `type` قرارداد اجرایی باید **دقیقاً** برابر `scope` باشد. آن را برابر `template_ref` قالبی بگذارید که تمدید با آن ساخته می‌شود.
- `cap`: سقف کل خرج زیر این وکالت (تومان). `per_use_cap`: سقف هر اجرا. `expires_at`: ISO، در آینده، ≤ ۳۶۶ روز.
- `label`: نام وکالت در فهرست کاربر و در پیام هر مصرف.
- `settlement: 'immediate'` و `app.action` برای اینکه ابزار تحویل صدا زده شود و `delegation_id` را بگیرید. پول جابه‌جا نمی‌شود (اثر پولی ندارد)؛ مبلغ قرارداد برای پله = `cap`.
