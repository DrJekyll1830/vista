...
seedApps();
if (config.sampleApp.enabled) registerSampleApp();
...
engine.setFulfiller(async (row, doc, platformSignature) => {
  ...
  const inst = q.get<any>('SELECT credential FROM installs WHERE user_id=? AND app_id=?', row.user_id, row.app_id);
...
setInterval(() => { engine.expireStale(); void engine.retryPending().catch(() => {}); }, 60_000);
