 ...
 */
export async function ledgerAppend(kind: string, payload: Record<string, unknown>, opts: { refType?: string; refId?: string; userId?: string; appId?: string } = {}) {
  const prev = (await q.get<{ hash: string }>('SELECT hash FROM ledger ORDER BY seq DESC LIMIT 1'))?.hash ?? 'genesis';
  const createdAt = now();
...
export async function ledgerVerify(): Promise<{ ok: boolean; count: number; brokenAt?: number }> {
  const rows = await q.all<{ seq: number; kind: string; payload_json: string; prev_hash: string; hash: string; created_at: string }>('SELECT * FROM ledger ORDER BY seq');
  let prev = 'genesis';
  for (const r of rows) {
