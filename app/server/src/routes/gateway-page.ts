...
gatewayPage.get('/:id', (c) => {
  const p = q.get<any>('SELECT * FROM payments WHERE id=?', c.req.param('id'));
...
gatewayPage.post('/:id/complete', async (c) => {
  ...
  try { completeTopup(pid, success, success ? id('ref', 10).toUpperCase() : undefined); } catch (e: any) { return c.html(shell('درگاه', `<h1>خطا</h1><p>${e?.message}</p>`), 400); }
