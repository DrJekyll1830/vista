...
appsApi.post('/app-events', async (c) => {
  ...
  const row = await engine.getContract(contract_id);
  ...
  if (!(await engine.verifyEventSignature(app_id, { contract_id, type, text, payload, ts }, signature))) return c.json({ error: 'signature', message: 'امضای اپ معتبر نیست' }, 403);
  if (!['executing', 'settled'].includes(row.status)) return c.json({ error: 'status', message: `قرارداد در وضعیت ${row.status} است` }, 409);
  try { const r = await engine.recordEvent(contract_id, type, 'app', app_id, text ?? '', payload ?? {}, signature); return c.json({ ok: true, status: r.status }); } catch (e: any) { return c.json({ error: e.code ?? 'error', message: e.message }, 400); }
});
appsApi.post('/app-actions/delegated', async (c) => {
  ...
  if (!s.success) return c.json({ error: 'bad_request', message: s.error.message }, 400);
  if (!await getApp(s.data.app_id)) return c.json({ error: 'not_found' }, 404);
  try { const r = await executeUnderDelegation(s.data.app_id, s.data.delegation_id, s.data.contract, s.data.app_signature); return c.json({ ok: true, contract_id: r.contract.id, status: r.contract.status, delegation: { spent: r.delegation.spent, cap: r.delegation.cap, status: r.delegation.status } }); }
