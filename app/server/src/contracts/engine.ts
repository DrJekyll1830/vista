...
async function appRow(appId: string) { return q.get<any>('SELECT * FROM apps WHERE id=?', appId); }
...
export async function verifyAppSignature(appId: string, hash: string, signature: string | undefined): Promise<{ ok: boolean; signer: string }> {
  const app = await appRow(appId);
  if (!app) return { ok: false, signer: 'unknown' };
  if (!signature) return { ok: false, signer: 'none' };
  const local = await q.get<{ public_key: string }>('SELECT public_key FROM app_keys WHERE app_id=?', appId);
  ...
  if (verifySig((await ensureKey('platform')).publicKey, hash, signature)) return { ok: true, signer: 'platform-on-behalf' };
 ...
 */
export async function createContract(userId: string, rawDoc: unknown, appSignature: string | undefined, opts: { origin?: 'assistant' | 'app' | 'system' | 'delegated'; delegationId?: string } = {}): Promise<ContractRow> {
  ...
  const app = await appRow(doc.app_id);
  if (!app) throw new ContractError('app_unknown', 'اپ سازندهٔ قرارداد شناخته نشده است.');
  if (await q.get('SELECT 1 FROM contracts WHERE id=?', doc.id)) throw new ContractError('dup', 'شناسهٔ قرارداد تکراری است.');
  if (await q.get('SELECT 1 FROM contracts WHERE nonce=?', doc.nonce)) throw new ContractError('nonce', 'یکتای قرارداد قبلاً استفاده شده است.');
  ...
  let signer = 'none';
  if (!isDraft) {
    const v = await verifyAppSignature(doc.app_id, hash, appSignature);
  ...
  await q.tx(async () => {
    await q.run(`INSERT INTO contracts (id, user_id, app_id, version, prev_version_id, type, template_ref, title, amount, status, doc_json, canonical_hash, nonce, required_rung, expires_at, origin, delegation_id, created_at)
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    ...
    if (!isDraft) {
      await q.run('INSERT INTO signatures (id, contract_id, version, party_id, party_kind, rung, hash, signature, created_at) VALUES (?,?,?,?,?,?,?,?,?)',
        id('sig'), doc.id, doc.version, `app:${doc.app_id}`, signer === 'platform-on-behalf' ? 'platform' : 'app', 1, hash, appSignature!, now());
    }
    await ledgerAppend('contract.created', { contractId: doc.id, version: doc.version, hash, status, appId: doc.app_id, amount, requiredRung: rung, origin: opts.origin ?? 'app' }, { refType: 'contract', refId: doc.id, userId, appId: doc.app_id });
  });
  const row = (await getContract(doc.id))!;
  if (opts.origin !== 'delegated') await chats.post(userId, doc.app_id, 'contract', doc.title, { contractId: doc.id, unread: opts.origin !== 'assistant' });
  return row;
}

export function getContract(cid: string): Promise<ContractRow | undefined> { return q.get<ContractRow>('SELECT * FROM contracts WHERE id=?', cid); }
export async function listContracts(userId: string, appId?: string) {
  return appId
...
export async function signaturesOf(cid: string) { return q.all<any>('SELECT id, party_id, party_kind, rung, hash, created_at, device FROM signatures WHERE contract_id=? ORDER BY created_at', cid); }
export async function eventsOf(cid: string) { return q.all<any>('SELECT * FROM events WHERE contract_id=? ORDER BY created_at', cid); }

/** آنچه کاربر می‌بیند — چیدمان مال سکوست؛ اپ فقط رنگ و نشان و نام می‌دهد. */
export async function projection(row: ContractRow, viewerPartyId: string) {
  const doc = docOf(row);
  const app = await appRow(doc.app_id);
  ...
  const sigs = await signaturesOf(row.id);
  ...
  return {
    ...
    events: (await eventsOf(row.id)).map((e) => ({ id: e.id, type: e.type, label: EVENT_LABELS[e.type] ?? e.type, text: e.text, actor: e.actor_kind, at: e.created_at, at_label: formatJalaliDateTime(e.created_at) })),
...
export async function checkSignable(row: ContractRow, userId: string) {
  ...
  if (row.expires_at && row.expires_at < now()) { await setStatus(row.id, 'expired'); throw new ContractError('expired', 'مهلت این قرارداد گذشته است.'); }
  const doc = docOf(row);
  ...
  if (await q.get('SELECT 1 FROM signatures WHERE contract_id=? AND version=? AND party_id=?', row.id, row.version, party.id)) throw new ContractError('dup_sig', 'شما قبلاً این نسخه را امضا کرده‌اید.');
  ...
  const app = await appRow(row.app_id);
...
export async function signContract(userId: string, cid: string, input: SignInput): Promise<ContractRow> {
  const row = await getContract(cid);
  if (!row) throw new ContractError('not_found', 'قرارداد یافت نشد.', 404);
  const party = await checkSignable(row, userId);
  if (input.rung < row.required_rung) throw new ContractError('rung', `این قرارداد به امضای پلهٔ ${row.required_rung} نیاز دارد.`);
  if (input.rung === 2) {
    const otp = input.otpId ? await q.get<any>('SELECT * FROM otps WHERE id=? AND purpose=? AND ref=? AND consumed_at IS NOT NULL', input.otpId, 'sign', row.canonical_hash) : null;
    if (!otp) throw new ContractError('otp', 'رمز یک‌بارمصرف این قرارداد تأیید نشده است.');
    if (await q.get('SELECT 1 FROM signatures WHERE otp_id=?', input.otpId)) throw new ContractError('otp_reused', 'این رمز قبلاً برای امضا استفاده شده است.');
  }
  const ts = now();
  ...
  const viewed = input.viewed ?? await projection(row, party.id);
  await q.tx(async () => {
    await q.run('INSERT INTO signatures (id, contract_id, version, party_id, party_kind, rung, hash, signature, otp_id, session_hint, device, viewed_json, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
      id('sig'), row.id, row.version, party.id, 'user', input.rung, row.canonical_hash, evidence, input.otpId ?? null, input.sessionToken.slice(-8), input.device ?? null, JSON.stringify(viewed), ts);
    await ledgerAppend('contract.signed', { contractId: row.id, version: row.version, party: party.id, rung: input.rung, hash: row.canonical_hash, viewed }, { refType: 'contract', refId: row.id, userId, appId: row.app_id });
  });
  // quorum: all must_sign parties
  const doc = docOf(row);
  const signed = new Set((await q.all<any>('SELECT party_id FROM signatures WHERE contract_id=? AND version=?', row.id, row.version)).map((s) => s.party_id));
  const missing = doc.parties.filter((p) => p.must_sign && !signed.has(p.id));
  if (missing.length === 0) {
    await setStatus(row.id, 'signed');
    return execute(row.id);
  }
  return (await getContract(cid))!;
}

export async function rejectContract(userId: string, cid: string, reason?: string) {
  const row = await getContract(cid);
  if (!row) throw new ContractError('not_found', 'قرارداد یافت نشد.', 404);
  ...
  await setStatus(cid, 'rejected');
  await ledgerAppend('contract.rejected', { contractId: cid, reason }, { refType: 'contract', refId: cid, userId, appId: row.app_id });
  await chats.post(userId, row.app_id, 'event', 'قرارداد رد شد', { replyToContractId: cid, unread: false });
  return (await getContract(cid))!;
}

export async function setStatus(cid: string, status: ContractStatus) {
  const extra = status === 'executing' || status === 'settled' ? ', executed_at = COALESCE(executed_at, ?)' : '';
  if (extra) await q.run(`UPDATE contracts SET status=?${extra}${status === 'settled' ? ', settled_at=?' : ''} WHERE id=?`, status, now(), ...(status === 'settled' ? [now()] : []), cid);
  else await q.run('UPDATE contracts SET status=? WHERE id=?', status, cid);
}

// ───────────────────────── events ─────────────────────────
/** رویداد — امضاشده است اما امضا روی قرارداد نیست. Applies wallet consequences and posts a reply in the app's chat. */
export async function recordEvent(cid: string, type: string, actorKind: 'app' | 'user' | 'platform', actorId: string, text = '', payload: Record<string, unknown> = {}, signature?: string) {
  const row = await getContract(cid);
  if (!row) throw new ContractError('not_found', 'قرارداد یافت نشد.', 404);
  const doc = docOf(row);
  const pay = doc.effects.find((e) => e.type === 'wallet.pay') as Extract<Effect, { type: 'wallet.pay' }> | undefined;
  const events = await eventsOf(cid);
  const held = events.some((e) => e.type === 'held') && !events.some((e) => ['deducted', 'released', 'cancelled', 'refunded'].includes(e.type));
  await q.tx(async () => {
    const eid = id('evt');
    await q.run('INSERT INTO events (id, contract_id, type, actor_kind, actor_id, text, payload_json, signature, created_at) VALUES (?,?,?,?,?,?,?,?,?)', eid, cid, type, actorKind, actorId, text, JSON.stringify(payload), signature ?? null, now());
    await ledgerAppend('contract.event', { contractId: cid, type, actorKind, actorId, text, payload }, { refType: 'contract', refId: cid, userId: row.user_id, appId: row.app_id });
    switch (type) {
      case 'delivered':
        if (pay && held) { await wallet.capture(row.user_id, pay.amount, cid, row.app_id, doc.title); await insertEvent(cid, 'deducted', 'platform', 'processor'); }
        await setStatus(cid, 'settled');
        break;
      case 'cancelled':
        if (pay && held) await wallet.release(row.user_id, pay.amount, cid, row.app_id);
        await setStatus(cid, 'cancelled');
        break;
      case 'failed':
        if (pay && held) await wallet.release(row.user_id, pay.amount, cid, row.app_id);
        await setStatus(cid, 'failed');
        break;
      case 'refunded':
        if (pay) await wallet.refund(row.user_id, Number(payload.amount ?? pay.amount), cid, row.app_id, doc.title);
        await setStatus(cid, 'refunded');
        break;
      case 'disputed':
        await setStatus(cid, 'disputed');
        break;
    }
  });
  const label = EVENT_LABELS[type] ?? type;
  await chats.post(row.user_id, row.app_id, 'event', text ? `${label} — ${text}` : label, { replyToContractId: cid, unread: actorKind !== 'user' });
  if (type === 'delivered' && pay && held) await chats.post(row.user_id, row.app_id, 'event', EVENT_LABELS.deducted, { replyToContractId: cid, unread: false });
  if (type === 'disputed') await chats.post(row.user_id, 'support', 'system', `اعتراض شما دربارهٔ «${doc.title}» ثبت شد و قرارداد در وضعیت «در اختلاف» است. پشتیبانی رسیدگی می‌کند.`, { replyToContractId: cid, unread: true });
  return (await getContract(cid))!;
}
async function insertEvent(cid: string, type: string, actorKind: string, actorId: string, text = '') {
  await q.run('INSERT INTO events (id, contract_id, type, actor_kind, actor_id, text, payload_json, created_at) VALUES (?,?,?,?,?,?,?,?)', id('evt'), cid, type, actorKind, actorId, text, '{}', now());
}
...
type EffectHook = (row: ContractRow, effect: Effect) => Promise<void>;
 ...
 */
export async function execute(cid: string): Promise<ContractRow> {
  const row = (await getContract(cid))!;
  const doc = docOf(row);
  ...
  if (row.expires_at && row.expires_at < now()) { await setStatus(cid, 'expired'); throw new ContractError('expired', 'قرارداد منقضی شده است.'); }
  ...
  const sigs = await q.all<any>('SELECT * FROM signatures WHERE contract_id=? AND version=?', cid, row.version);
  for (const p of doc.parties.filter((p) => p.must_sign)) {
    ...
    if (p.kind === 'app' && !(await verifyAppSignature(doc.app_id, hash, s.signature)).ok) throw new ContractError('app_signature', 'امضای اپ معتبر نیست.');
  }
  // 4. conditions
  for (const c of doc.conditions) {
    ...
    if (c.type === 'delegation.active') {
      const d = await q.get<any>('SELECT * FROM delegations WHERE id=?', c.delegation_id);
  ...
  try {
    await q.tx(async () => {
      for (const e of doc.effects) {
        switch (e.type) {
          case 'wallet.pay': {
            await wallet.hold(row.user_id, e.amount, cid, row.app_id);
            await insertEvent(cid, 'held', 'platform', 'processor');
            if (doc.policy.settlement === 'immediate') {
              await wallet.capture(row.user_id, e.amount, cid, row.app_id, doc.title);
              await insertEvent(cid, 'deducted', 'platform', 'processor');
            }
            for (const f of doc.fees) await ledgerAppend('fee.recorded', { contractId: cid, beneficiary: f.beneficiary, amount: f.amount }, { refType: 'contract', refId: cid, userId: row.user_id, appId: row.app_id });
            break;
          }
          ...
          case 'app.install': case 'permission.grant': case 'delegation.grant': case 'genesis': case 'app.action': {
            if (effectHooks[e.type]) await effectHooks[e.type]!(row, e);
            break;
          }
        }
      }
      await ledgerAppend('contract.executed', { contractId: cid, hash, effects: doc.effects.map((e) => e.type) }, { refType: 'contract', refId: cid, userId: row.user_id, appId: row.app_id });
      await setStatus(cid, 'executing');
    });
  } catch (e: any) {
    const msg = e instanceof WalletError || e instanceof ContractError ? e.message : 'اجرا ناموفق بود';
    await setStatus(cid, 'failed');
    await insertEvent(cid, 'failed', 'platform', 'processor', msg);
    await ledgerAppend('contract.failed', { contractId: cid, reason: msg }, { refType: 'contract', refId: cid, userId: row.user_id, appId: row.app_id });
    await chats.post(row.user_id, row.app_id, 'event', `${EVENT_LABELS.failed} — ${msg}`, { replyToContractId: cid });
    throw new ContractError('exec_failed', msg);
  }
  const pay = doc.effects.find((e) => e.type === 'wallet.pay');
  if (pay) await chats.post(row.user_id, row.app_id, 'event', doc.policy.settlement === 'immediate' ? EVENT_LABELS.deducted : EVENT_LABELS.held, { replyToContractId: cid, unread: false });
  // nothing more to wait for?
  const needsApp = !!fulfiller && doc.effects.some((e) => e.type === 'wallet.pay' || e.type === 'app.action' || e.type === 'delegation.grant');
  if (!needsApp && !pending) await setStatus(cid, 'settled');
...
export async function fulfil(cid: string) {
  ...
  const doc = docOf(row);
  const app = await appRow(row.app_id);
  if (!app || app.kind === 'system') { if (row.status === 'executing' && !doc.effects.some((e) => e.type === 'wallet.topup')) await setStatus(cid, 'settled'); return; }
  try {
    const res = await fulfiller(row, doc, await platformSign(`executed|${row.canonical_hash}`));
    if (!res) { if (row.status === 'executing' && doc.policy.settlement === 'immediate') await setStatus(cid, 'settled'); return; }
    for (const ev of res.events ?? []) await recordEvent(cid, ev.type, 'app', row.app_id, ev.text ?? '', ev.payload ?? {});
    const fresh = (await getContract(cid))!;
    ...
    if (fresh.status === 'executing' && doc.policy.settlement === 'immediate') await setStatus(cid, 'settled');
  } catch (e: any) {
    console.error('[fulfil]', cid, e?.message ?? e);
    await insertEvent(cid, 'note', 'platform', 'processor', 'اپ در لحظهٔ اجرا پاسخ نداد؛ دوباره تلاش می‌شود.');
    await chats.post(row.user_id, row.app_id, 'event', 'اپ در لحظهٔ اجرا پاسخ نداد؛ دوباره تلاش می‌شود.', { replyToContractId: cid, unread: false });
  }
}
/** Retry contracts still waiting on their app. */
export async function retryPending() {
  const rows = await q.all<ContractRow>("SELECT * FROM contracts WHERE status='executing' AND executed_at < ? AND executed_at > ?", new Date(Date.now() - 60_000).toISOString(), new Date(Date.now() - 24 * 3_600_000).toISOString());
  for (const r of rows) {
    const doc = docOf(r);
    if (doc.effects.some((e) => e.type === 'wallet.topup')) continue;
    if (!doc.effects.some((e) => e.type === 'wallet.pay' || e.type === 'app.action')) { await setStatus(r.id, 'settled'); continue; }
    await fulfil(r.id);
  }
}
/** Expire awaiting contracts past their deadline. */
export async function expireStale() {
  const rows = await q.all<ContractRow>("SELECT * FROM contracts WHERE status IN ('awaiting','draft') AND expires_at < ?", now());
  for (const r of rows) { await setStatus(r.id, 'expired'); await chats.post(r.user_id, r.app_id, 'event', 'مهلت قرارداد گذشت و منقضی شد', { replyToContractId: r.id, unread: false }); }
}

/** Signed event envelope check for app-posted events. */
export async function verifyEventSignature(appId: string, body: { contract_id: string; type: string; text?: string; payload?: unknown; ts: string }, signature: string) {
  return (await verifyAppSignature(appId, canonicalJson({ contract_id: body.contract_id, type: body.type, text: body.text ?? '', payload: body.payload ?? {}, ts: body.ts }), signature)).ok;
}
export { json, toman };
